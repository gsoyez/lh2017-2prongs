...
#resilience ="1.0/sqrt((column('Bcorrection_'.level)/(column('Bcorrection_'.level)+2))**2+(column('Scorrection_'.level)/(column('Scorrection_'.level)+2))**2)"
resilience ="1.0/sqrt(column('Bcorrection_'.level)**2+column('Scorrection_'.level)**2)"
performance="column('significance_'.levelref)"

do for [R in "0.8 1.0"]{
    set out 'default-perf-R'.R.'.pdf'
    ...
    if ((R+0.0)<0.9){
        ...
        set label 51 '{/*0.85  500}' at 2.4,2.5
        set label 52 '{/*0.85 1000}' at 1.85,3.15
        set label 53 '{/*0.85 2000}' at 0.9,4.6
        set label 54 '{/*0.85  500}' at 0.85,3.35
        set label 55 '{/*0.85 1000}' at 0.83,3.85
    }
