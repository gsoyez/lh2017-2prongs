%% GS todo:
%%
%% - Fig.7 needs to be updated to include the 3 benchmark points and
%%   R=0.8 only.
%% 
%% 
%% - the "(dichroic)" label in the two leftmost plots of Figs.9 and 11 is
%%   a bit of a misnomer. We could rename them as "(LH[D]D2)", possibly
%%   with a / across the 1st D? [and a similar renaminig for Figs 8 and
%%   10. What do you think?
%% 
%% - Section 5.3 is. to me the main take-home message of the
%%   proceedings. Is there a way to highlight this better?
%% 
%% - Fig. 12: highlight "our pick" [this one is obviously on me]
%% 
%% - Detector sim: what about adding a link to Peter's code (adding some
%%   piece of info there to mention that he is the author of the code)?
%% 
%% - Fig. 13: highlight "LHDD22"
%% 
%% - For Peter, what do the two terms in (23) (track smearing) correspond
%%   to?
%% 
%% - Section 7.1, figure 15a: specify the grooming strategy for D2
...
% 
% The $N_2$ observable has been used by CMS, and the $D_2$ observable
% has been used by ATLAS.
%
...
%
% the SoftDrop algorithm is defined
...
%
% Given a jet that has been groomed with the SoftDrop procedure, we
% can then measure any IRC-safe observable on this jet and it is IRC
% safe.
%
...
%  \qquad
%  \tau_{21}^{\text{groomed}} =\frac{\tau_2^{\text{groomed}}}{\tau_1^{\text{groomed}}}\,,
%   \qquad
%   \tau_{21}^{\text{dichroic}} =\frac{\tau_2}{\tau_1^\text{groomed}}\,,
% \end{align}
%
\begin{align}
...
\end{align}
%
% The definitions of the variants of the $M_2$, $N_2$ and $D_2$
% observables are then
% \begin{align}
% M_2= \frac{ _1e_{3}  }{e_{2}}\,, \qquad  M_2^{\text{groomed}}= \frac{
%   ( _1e_{3})^{\text{groomed}}  }{e_{2}^\text{groomed}}\,, \qquad  M_2^{\text{dichroic}}= \frac{_1e_{3}  }{e_{2}^\text{groomed}}\,, 
% \end{align}
% \begin{align}
...
% Due to the large number of physics and detector parameters that are
% varied in our study, only a subset of plots will be presented in the
% paper. A summary of the observables scanned, highlighting those
% which we find to be most promising, will be given in
% Fig.~\ref{jetsub_2prong_fig:phasespace}.
%
...
%
%  Among the shapes, we find that $D_2$ tends to be the most robust,
%  while $N_2$ tends to be slightly more performant.
%
...
% With the exception of the tight$\otimes$plain/plain grooming strategy
% (which indeed does not groom the observable), all the standard
% observables with are robust to underlying event for all the different
% grooming strategies.
% %
% Similarly, with the exception of the dichroic $D_2$ observable, all
% the different jet shape observables in this study are also robust to
% underlying event.
%
...
%
% Since the standard two-prong tagging observables are not particularly
% sensitive to the polarization, a different observable must be used to
% tag polarizations.
%
