...
\section{PERFORMANCE VERSUS ROBUSTNESS:  \\ TWO-PRONG SUBSTRUCTURE TAGGERS FOR THE LHC \protect\footnote{Section coordinators:  P.~Loch, I.~Moult, B.~Nachman, G.~Soyez, J.~Thaler}$^{,}$~\protect\footnote{Contributing authors: D.~Bhatia, R.~Camacho, G.~Chachamis, S.~Chatterjee, F.~Dreyer, D.~Kar, A.~Papaefstathiou, T.~Samui, A.~Si\'{o}dmok}}
...
Similarly, \ensuremath{a_{\text{track}}}{} and \ensuremath{c_{\text{track}}}{} are used in Eq.~\eqref{jetsub_2prong_eq:trkreso} to determine the width of the track-\ensuremath{p_{T}}{} resolution smearing.}
...
The selection employing $\ensuremath{p_{T}^{\text{tower}}} > \ensuremath{p_{T}^{\text{min}}}$ is applied as well after the tower energy smearing with the parameters for configuration C given in Table~\ref{jetsub_2prong_tab:detmodel}. 
